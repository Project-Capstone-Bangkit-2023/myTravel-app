...
class RecommendAdapter(private val listRecommend: List<Recommend>) : RecyclerView.Adapter<RecommendAdapter.ListViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ListViewHolder {
        val view: View = LayoutInflater.from(parent.context).inflate(R.layout.item_recommend, parent, false)
    ...
    override fun onBindViewHolder(holder: ListViewHolder, position: Int) {
        val (username, photo, url, weather) = listRecommend[position]
        Glide.with(holder.itemView.context)
            .load(photo)
            .into(holder.imgPhoto)
        holder.tvName.text = username
        holder.tvAddress.text = url
